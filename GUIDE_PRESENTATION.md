...
Ce document est votre "antisèche" pour une présentation fluide, étape par étape.
...
*   **Action :** Montrez le fichier `IUtilisateurService.java`.
*   **Points clés à dire :**
...
*   **Action :** Montrez `Utilisateur.java` puis `UtilisateurServiceImpl.java`.
*   **Points clés à dire :**
    1.  "Notre modèle est annoté avec `@XmlRootElement`. C'est crucial pour que JAX-WS sache comment transformer nos objets Java en XML (c'est ce qu'on appelle le Marshalling)."
...
*   **Action :** Lancez `UtilisateurPublisher.java` et ouvrez le navigateur sur `http://localhost:8080/ws/utilisateurs?wsdl`.
*   **Points clés à dire :**
...
*   **Action :** Lancez `UtilisateurClient.java`.
*   **Points clés à dire :**
    ...
    2.  **Démonstration :**
        *   Faites une **Liste (1)** pour montrer les données de départ.
        *   Faites un **Ajout (3)** : Insistez sur le fait que vous ne saisissez pas de matricule. Montrez le message de succès avec le matricule généré par le serveur.
        *   Faites une **Recherche (2)** avec le matricule que vous venez de créer.
        *   Faites une **Modification (4)** ou **Suppression (5)** pour prouver que l'état est maintenu sur le serveur.

---

### ⚖️ ÉTAPE 5 : COMPARAISON THÉORIQUE (RMI vs SOAP)
*   **Question probable du jury :** "Pourquoi SOAP est-il mieux que RMI ?"
*   **Votre réponse type :**
...
3.  **Calme :** Si le serveur plante, expliquez simplement qu'il faut le relancer (c'est courant avec les ports réseau).
